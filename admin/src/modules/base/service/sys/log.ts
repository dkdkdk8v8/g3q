...
export class BaseSysLogService extends BaseService {
   ...
   * @param userId 用户ID
   */
  async record(context: Context, url: string, params, userId) {
    const ip = await this.utils.getReqIP(context);
    const ipStr = typeof ip === 'string' ? ip : ip.join(',');
    if (ipStr === '127.0.0.1') return; // 不记录测试环境
    const action = url.split('?')[0];
    if (action.indexOf('/open/') === 0) return; // 不记录内部访问接口
