<template>
	<div class="game-room-qznn">
		...
		<div class="stats-bar">
			<div class="group">
				<span v-for="(count, name) in statsData.levels" :key="name" class="item">{{ name }} <span class="num">{{ count }}</span></span>
			</div>
			<div class="divider"></div>
			<div class="group">
				<span v-for="(count, name) in statsData.types" :key="name" class="item">{{ name }} <span class="num">{{ count }}</span></span>
			</div>
		</div>
					...
					<template #header>
						<div class="card-header">
							<div class="header-left">
								<div class="tags">
									<el-tag size="small" type="danger" effect="plain">{{ getRoomInfo(item.ID).level }}</el-tag>
									<el-tag size="small" type="warning" effect="plain">{{ getRoomInfo(item.ID).type }}</el-tag>
								</div>
							</div>
						...
						<div class="players-list">
							...
							>
								<div v-if="player" class="player-info">
									<el-avatar :size="36" :src="player.Avatar" :icon="UserFilled" class="avatar" :class="{ 'is-banker': item.BankerID === player.ID }"></el-avatar>
									<div class="details">
										<el-tooltip
											effect="dark"
									...
									<div class="player-balance">￥{{ (player.Balance / 100).toFixed(2) }}</div>
								</div>
...
const stateMap: Record<string, string> = {
	"QZNN.StateWaiting": "等待中",
	"QZNN.StatePrepare": "即将开始",
	"QZNN.StatePreCard": "预发牌",
	"QZNN.StateBanking": "抢庄中",
	"QZNN.StateRandomBank": "随机抢庄",
	"QZNN.StateBankerConfirm": "庄家确认",
	"QZNN.StateBetting": "下注中",
	"QZNN.StateDealing": "发牌中",
	"QZNN.StateShowCard": "亮牌中",
	"QZNN.StateSettling": "结算中",
};

const stateTypeMap: Record<string, string> = {
	"QZNN.StateWaiting": "info",
	"QZNN.StatePrepare": "info",
	"QZNN.StatePreCard": "primary",
	"QZNN.StateBanking": "warning",
	"QZNN.StateRandomBank": "warning",
	"QZNN.StateBankerConfirm": "warning",
	"QZNN.StateBetting": "success",
	"QZNN.StateDealing": "primary",
	"QZNN.StateShowCard": "primary",
	"QZNN.StateSettling": "danger",
};
...
.game-room-qznn {
	...
	.stats-bar {
		...
		.group {
			...
			align-items: center;
			margin-right: 20px;
			.label { font-weight: bold; margin-right: 10px; color: #303133; }
			.item { margin-right: 15px; .num { color: #409eff; font-weight: bold; margin-left: 4px; } }
		}
