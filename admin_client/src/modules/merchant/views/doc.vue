...
const apiList = [
  {
    ...
    notes: [
      ...
      'mode=html 模式：返回的 HTML 内嵌 JS 会并发测速所有线路，最先响应的线路将被自动选中并跳转；若 5 秒内均未响应，则随机选择一条线路',
    ],
  },
  {
    id: 'sec-balance',
  ...
  },
  {
    ...
    response: [
      ...
      { name: 'list[].gameData', type: 'object', desc: '游戏详细数据，无数据时为空对象 {}' },
      { name: 'list[].gameData.gameId', type: 'string', desc: '游戏局号' },
      { name: 'list[].gameData.bankerID', type: 'string', desc: '庄家商户侧玩家标识' },
      { name: 'list[].gameData.players', type: 'array', desc: '参与玩家列表' },
      { name: 'list[].gameData.players[].id', type: 'string', desc: '商户侧玩家标识' },
      { name: 'list[].gameData.players[].nickName', type: 'string', desc: '玩家昵称' },
      { name: 'list[].gameData.players[].seatNum', type: 'number', desc: '座位号' },
      { name: 'list[].gameData.players[].cards', type: 'array', desc: '手牌（数字编码）' },
      { name: 'list[].gameData.players[].callMult', type: 'number', desc: '抢庄倍数' },
      { name: 'list[].gameData.players[].betMult', type: 'number', desc: '下注倍数' },
      { name: 'list[].gameData.players[].balanceChange', type: 'number', desc: '输赢金额（分）' },
      { name: 'list[].gameData.players[].validBet', type: 'number', desc: '有效投注额（分）' },
      { name: 'list[].gameData.config', type: 'object', desc: '房间配置' },
      { name: 'list[].gameData.config.level', type: 'number', desc: '房间等级' },
      { name: 'list[].gameData.config.name', type: 'string', desc: '房间名称' },
      { name: 'list[].gameData.config.baseBet', type: 'number', desc: '底注（分）' },
    ...
    notes: [
      ...
      '不传时间范围时查询全部记录',
      '记录按时间倒序排列',
      'gameData 仅在抢庄牛牛（qznn/qznn3/qznn4）游戏中包含详细数据，其他游戏返回空对象 {}',
    ],
  },
  {
    id: 'sec-fundRecords',
