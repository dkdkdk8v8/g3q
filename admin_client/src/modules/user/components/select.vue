<template>
	<div class="user-select__inner">
		<template v-if="multiple">
			<div class="btns">
				...
				<el-button type="danger" :disabled="refs.table?.selection.length == 0" @click="remove">移除</el-button>
			</div>

			<cl-crud padding="0">
				<cl-table size="small" :data="data" :ref="setRefs('table')" :auto-height="false" />

				<cl-row type="flex" align="middle" justify="end" :style="{ marginTop: '10px' }">
					<el-pagination v-model:current-page="pager.page" :page-size="pager.size" :total="list.length"
	...
	<cl-dialog v-model="visible" width="1200px" title="选择用户">
		<cl-crud ref="Crud" padding="0">
			...
			<cl-row>
				<!-- 数据表格 -->
				<cl-table size="small" ref="Table" :auto-height="false" @selection-change="onSelectionChange">
					<template #column-check="{ scope }">
						<el-button type="success" disabled v-if="selection[0]?.id == scope.row.id">已选</el-button>
		...
		<template #footer>
			...
			<el-button type="success" :disabled="isEmpty(selection)" @click="select()" v-if="multiple">选择</el-button>
		</template>
	</cl-dialog>
</template>
